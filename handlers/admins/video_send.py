...
from states.my_states import VideoSend
...
async def send_message(message: types.Message, state: FSMContext):
    ...
    if data.get('buttons', None):
        ...
        for i in users:
            try:
                await bot.send_video(chat_id=i[0], video=data['video'], reply_markup=btn.as_markup(row_width=1), caption=data['caption'])
                counter += 1
        ...
        for i in users:
            try:
                await bot.send_video(video=data['video'], chat_id=i[0])
                counter += 1
            except Exception as e:
                print(e)
        await message.answer(f"{counter} kishiga xabar yuborildi!", reply_markup=admin_button())
