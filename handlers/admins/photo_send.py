...
from states.my_states import PhotoSend
...
async def send_message(message: types.Message, state: FSMContext):
    ...
    if data.get('buttons', None):
        ...
        for i in users:
            try:
                await bot.send_photo(chat_id=i[0], photo=data['photo'], reply_markup=btn.as_markup(row_width=1), caption=data['caption'])
                counter += 1
        ...
        for i in users:
            try:
                await bot.send_photo(photo=data['photo'], chat_id=i[0])
                counter += 1
            except Exception as e:
                print(e)
        await message.answer(f"{counter} kishiga xabar yuborildi!", reply_markup=admin_button())
